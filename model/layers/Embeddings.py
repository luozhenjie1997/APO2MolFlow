...
from model.Track_module import PairStr2Pair, PositionalEncoding2D
from model.utils.util_module import Dropout, get_clones, create_custom_forward, rbf, init_lecun_normal, get_res_atom_dist
from model.utils.chemical import NBTYPES
...
class MSA_emb(nn.Module):
    ...
    def __init__(self, d_msa=256, d_pair=128, d_state=32, d_init=0,
        ...
        if (d_init==0):
            d_init = 2 * chemical.NAATOKENS+2+2
            
        super(MSA_emb, self).__init__()
        self.emb = nn.Linear(d_init, d_msa) # embedding for general MSA
        self.emb_q = nn.Embedding(chemical.NAATOKENS, d_msa) # embedding for query sequence -- used for MSA embedding
        self.emb_left = nn.Embedding(chemical.NAATOKENS, d_pair) # embedding for query sequence -- used for pair embedding
        self.emb_right = nn.Embedding(chemical.NAATOKENS, d_pair) # embedding for query sequence -- used for pair embedding
        self.emb_state = nn.Embedding(chemical.NAATOKENS, d_state)
        ...
        self.bond_pair_weight = nn.Parameter(torch.empty(NBTYPES, d_pair))
    ...
    def reset_parameter(self):
        ...
        nn.init.trunc_normal_(self.bond_pair_weight, std=0.02)
    ...
    def _pair_emb(self, seq, idx, bond_feats, dist_matrix, sm_mask, is_protein, is_atomize_protein, same_chain=None, seq1hot=None):
        if seq1hot is not None:  # 将离散的序列选择问题转化为连续的分布优化问题，并允许端到端的梯度优化
        ...
        pair = pair + self.pos(idx, bond_feats, dist_matrix, sm_mask, is_protein, is_atomize_protein, same_chain=same_chain)  # add relative position
    ...
    def forward(self, msa, seq, idx, bond_feats, dist_matrix, sm_mask, is_protein, is_atomize_protein, same_chain=None,
                seq1hot=None, bond_noisy=None):
        # Inputs:
        ...
        # 初始化成对特征
        pair = self._pair_emb(seq, idx, bond_feats, dist_matrix, sm_mask, is_protein, is_atomize_protein, same_chain=same_chain, seq1hot=seq1hot)
        if bond_noisy is not None:
            pair_bond = bond_noisy @ self.bond_pair_weight
        else:
            pair_bond = 0.0
        pair = pair + pair_bond  # 将含有噪声的化学键类型特征集成进去
        # state embedding
...
class Extra_emb(nn.Module):
    ...
    def __init__(self, d_msa=256, d_init=0, p_drop=0.1):
        ...
        if d_init==0:
            d_init=chemical.NAATOKENS + 2
        self.emb = nn.Linear(d_init, d_msa) # embedding for general MSA
        self.emb_q = nn.Embedding(chemical.NAATOKENS, d_msa) # embedding for query sequence
    ...
    def forward(self, msa, seq, seq1hot=None):
        # Inputs:
        #   - msa: Input MSA (B, N, L, d_init)
        #   - seq: Input Sequence (B, L)
        # Outputs:
...
class Bond_emb(nn.Module):
    ...
    def forward(self, bond_feats):
        bond_feats = torch.nn.functional.one_hot(bond_feats, num_classes=chemical.NBTYPES)
...
class Templ_emb(nn.Module):
    ...
    def __init__(self, d_t1d=0, d_t2d=67+1, d_tor=0, d_pair=128, d_state=32, 
                 ...
                 n_head=4, d_hidden=16, d_time_emb=None, p_drop=0.25,
        ...
        if d_t1d == 0:
            d_t1d = (chemical.NAATOKENS-1)+1
        if d_tor == 0:
            d_tor = 3 * chemical.NTOTALDOFS
        if d_time_emb is not None:
            d_t1d += d_time_emb
    ...
    def forward(self, t1d, t2d, alpha_t, xyz_t, mask_t, pair, state, use_checkpoint=False, p2p_crop=-1):
        ...
        t1d = torch.cat((t1d, alpha_t.reshape(B, T, L, -1)), dim=-1)  # (B, T, L, 30+3*17)
        # process each template features
...
class RecyclingAllFeatures(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, d_state=32, d_rbf=64):
        ...
        self.norm_pair = nn.LayerNorm(d_pair)
        self.proj_sctors = nn.Linear(2*chemical.NTOTALDOFS, d_msa)
        self.norm_msa = nn.LayerNorm(d_msa)
    ...
    def forward(self, msa, pair, xyz, state, sctors, mask_recycle=None):
        ...
        sctors = self.proj_sctors(sctors.reshape(B,-1,2*chemical.NTOTALDOFS))
