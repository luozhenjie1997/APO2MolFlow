...
from .chemical import aachirals
...
def get_chirals(obmol, xyz):
    '''
    获取所有构成手性中心且预期具有理想伪二面角的原子四元组（得到的原子索引是从0开始的）
    '''
    stereo = openbabel.OBStereoFacade(obmol)
    angle = np.arcsin(1/3**0.5)  # 理想四面体角度
    chiral_idx_set = set()
...
def get_chiral_tags_from_mol(sdf_str):
    mol = Chem.MolFromMolBlock(sdf_str, removeHs=True, sanitize=True)
    ...
    for atom_idx, _ in chiral_centers:
        ...
        # 如果模型不含氢且邻居不足 3 个，无法计算体积，跳过
        if len(neighbors) < 3:
            continue

        # 选定前 3 个邻居即可。只要我们在计算 vol_true 和 vol_pred 时，
        # 邻居的相对顺序严格保持一致，真实坐标的体积就会成为绝对的基准！
        chiral_tags.append([atom_idx, neighbors[0], neighbors[1], neighbors[2]])

    return chiral_tags

def get_atomize_protein_chirals(residues_atomize, lig_xyz, residue_atomize_mask, bond_feats):
    """
    枚举残基中的手性中心并提供手性中心的特征信息
    """
